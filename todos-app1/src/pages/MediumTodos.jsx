...
const MediumTodos=() => {
  ...
  useEffect(()=> {
    const fetctAllTodos= async() => {
        try {
            const res1=await axios.get("http://localhost:8800/mediumpendingtodos")
            setPendingTodos(res1.data)

            const res2=await axios.get("http://localhost:8800/mediumcompletedtodos")
