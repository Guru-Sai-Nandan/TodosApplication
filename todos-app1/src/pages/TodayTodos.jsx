...
const TodayTodos=() => {
  ...
  useEffect(()=> {
    const fetctAllTodos= async() => {
        try {
            const res1=await axios.get("http://localhost:8800/todaypendingtodos")
            setPendingTodos(res1.data)

            const res2=await axios.get("http://localhost:8800/todaycompletedtodos")
  ...
  const handlePendingStatus=async(todo)=> {
    try {
        ...
        const res1=await axios.get("http://localhost:8800/pendingtodos")
        setPendingTodos(res1.data)
        const res2=await axios.get("http://localhost:8800/completedtodos")
        setCompletedTodos(res2.data)
        // window.location.reload()
    }
    catch(err) {
